...
export ASCEND_RT_VISIBLE_DEVICES="${ASCEND_RT_VISIBLE_DEVICES_DECODE}"
export HCCL_BUFFSIZE="${DECODE_HCCL_BUFFSIZE:-1024}"

export VLLM_DP_SIZE=1
export VLLM_DP_MASTER_IP="${DECODE_DP_MASTER_IP:-127.0.0.1}"
export VLLM_DP_MASTER_PORT="${DECODE_DP_MASTER_PORT:-13495}"
...
echo "$(date '+%Y-%m-%d %H:%M:%S') [decode] ASCEND_RT_VISIBLE_DEVICES=${ASCEND_RT_VISIBLE_DEVICES}"
echo "$(date '+%Y-%m-%d %H:%M:%S') [decode] MODEL_PATH=${MODEL_PATH}"
echo "$(date '+%Y-%m-%d %H:%M:%S') [decode] TYPE=${TYPE}"
echo "$(date '+%Y-%m-%d %H:%M:%S') [decode] TURBOQUANT_KV_BITS=${TURBOQUANT_KV_BITS:-N/A}"
