...
def main() -> None:
    ...
    prompt_tokens = _int_env("XRX_FIA_LONG_PROMPT_TOKENS", 512)
    num_prompts = _int_env("XRX_FIA_LONG_NUM_PROMPTS", 1)
    max_tokens = _int_env("XRX_FIA_LONG_OUTPUT_TOKENS", 2)
    max_model_len = _int_env(
        "XRX_FIA_LONG_MAX_MODEL_LEN",
        max(256, prompt_tokens + max_tokens + 32),
    )
    gpu_memory_utilization = _float_env("XRX_FIA_GPU_MEMORY_UTILIZATION", 0.10)
