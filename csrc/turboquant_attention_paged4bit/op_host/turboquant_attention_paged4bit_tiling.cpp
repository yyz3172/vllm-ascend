...
#include "turboquant_attention_paged4bit_tiling.h"
...
constexpr uint32_t TQ_ROT_N = optiling::TQ_ATTN_HEAD_SIZE;
constexpr uint32_t TQ_ROT_K = optiling::TQ_ATTN_HEAD_SIZE;
constexpr uint32_t TQ_ROT_MAX_BASE_M = optiling::TQ_ATTN_UB_KV_TILE_CAP;
...
constexpr uint32_t FLASH_DECODE_MIN_KV_LEN = 1024;
...
bool IsFlashDecode(uint32_t taskCount, uint32_t maxKvLen, uint32_t aicNum)
{
    if (maxKvLen < FLASH_DECODE_MIN_KV_LEN) {
        return false;
    }
    if (taskCount > static_cast<uint32_t>(FLASH_DECODE_BN_RATIO * aicNum)) {
        return false;
    }
    return true;
}
    ...
    optiling::TCubeTiling& cubeTiling)
{
    ...
    // Match the pack v2 / 4bit KFC contract: A and C are both local L1
    // tensors, so keep baseM within the rows that fit A[M,K] + C[M,N].
...
uint64_t PickTilingKey(uint32_t splitMode, uint32_t qkPvMode, bool qTileMode,
                       uint32_t gqaGroup)
{
    if (qTileMode) {
        if (gqaGroup <= optiling::TQ_ATTN_QTILE_SMALL_GQA_CAP) {
            return optiling::TQ_ATTN_KEY_SPLITBN_VECTOR_QTILE_GQA2;
        }
        return optiling::TQ_ATTN_KEY_SPLITBN_VECTOR_QTILE;
    }
...
size_t CalcWorkspaceSize(const platform_ascendc::PlatformAscendC& platform, uint32_t splitMode,
                         ...
                         uint32_t kvSplitPart, uint32_t launchedBlockDim)
{
    // Match IFA: lib-api scratch for the decode/QK/PV matmul path plus optional
    // user buffers for FlashDecode partial outputs.
    size_t ws = CalcSystemWorkspaceSize(platform);
    if (splitMode == optiling::TQ_ATTN_SPLIT_BNS) {
        const size_t parts = static_cast<size_t>(numTokens) * numHeads * kvSplitPart;
        const size_t accumOutElems = parts * headSize;
        const size_t lseBytes = parts * 2 * sizeof(float);
        ws += accumOutElems * sizeof(float) + lseBytes;
    }
    return ws;
}
...
static ge::graphStatus TurboquantAttentionPaged4bitTilingFunc(gert::TilingContext* context)
{
    if (context == nullptr) {
    ...
    auto codebookDesc = context->GetInputDesc(6);
    auto rotationDesc = context->GetInputDesc(7);
    auto codebookValueDesc = context->GetInputDesc(8);
    auto rotationValueDesc = context->GetInputDesc(9);
    ...
    if (qShape.GetDimNum() < 2 || cacheShape.GetDimNum() < 3 || btShape.GetDimNum() < 2) {
        OPS_LOG_E(nodeName, "unexpected input ranks");
        return ge::GRAPH_FAILED;
    }
    if (queryDesc == nullptr || codebookDesc == nullptr || rotationDesc == nullptr ||
        codebookValueDesc == nullptr || rotationValueDesc == nullptr) {
    ...
    if ((dataType != ge::DT_FLOAT16 && dataType != ge::DT_BF16) ||
        ...
        codebookValueDesc->GetDataType() != dataType ||
        rotationValueDesc->GetDataType() != dataType) {
        OPS_LOG_E(nodeName, "query/codebook/rotation/codebook_value/rotation_value must share fp16 or bf16 dtype");
        return ge::GRAPH_FAILED;
    }
    ...
    if (cacheShape.GetDim(1) != static_cast<int64_t>(numKvHeads) ||
        cacheShape.GetDim(2) != static_cast<int64_t>(blockSize * TQ_ATTN_PACKED_BYTES)) {
        OPS_LOG_E(nodeName,
                  "key_cache shape must be [num_blocks, num_kv_heads, block_size * %u]",
    ...
    const uint32_t gqaChunkCount = (gqaGroup + TQ_ATTN_UB_GQA_CAP - 1) / TQ_ATTN_UB_GQA_CAP;
    ...
    TurboquantAttentionPaged4bitTilingData tiling{};
    if (FillDecodeRotateTiling(nodeName, ascendcPlatform, kvTileRows, mmDataType,
                               tiling.decodeRotateTiling) !=
        ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }

    // TODO(turboquant): re-enable FlashDecode only as a two-stage partial/merge
    // launch, or with a proven device-side global barrier. The single-kernel
    // SplitBNS prototype makes core0 combine partials written by other cores,
    // which violates the self-contained core execution model and is not
    // correctness-safe on Ascend.
    const bool splitBns = false;
    const uint32_t splitMode = splitBns ? TQ_ATTN_SPLIT_BNS : TQ_ATTN_SPLIT_BN;
    // TODO(turboquant): prototype QK/PV Cube in a separate key/TU, following
    // sparse_flash_attention's split cube/vector services, so registering extra
    // Matmul objects does not inflate Mc2 static workspace for the vector path.
    const uint32_t qkPvMode = TQ_ATTN_QKPV_VECTOR;
    ...
    if (splitMode == TQ_ATTN_SPLIT_BNS) {
        SplitBns(taskCount, maxActualSeqLen, blockSize, parallelCoreNum, usedCoreNum, kvSplitPart,
    ...
    const bool qTileMode = splitMode == TQ_ATTN_SPLIT_BN &&
                           qkPvMode == TQ_ATTN_QKPV_VECTOR &&
                           blockSplitRange > headChunkScale;
    ...
    if (splitMode == TQ_ATTN_SPLIT_BNS) {
        ...
        tiling.set_partialWorkspaceOffset(CalcSystemWorkspaceSize(ascendcPlatform));
    } else {
    ...
    // Keep the launched MIX group count stable, matching the v2 pack kernel.
    // Idle AIV workers return in-kernel after the usedCoreNum check.
    ...
    workspaces[0] = CalcWorkspaceSize(ascendcPlatform, splitMode, numTokens, numHeads, headSize,
                                      kvSplitPart, blockDim);
    ...
    const uint64_t tilingKey = PickTilingKey(splitMode, qkPvMode, qTileMode, gqaGroup);
    context->SetTilingKey(tilingKey);
    OPS_LOG_I(nodeName,
              "TurboquantAttentionPaged4bit tiling: tokens=%u tasks=%u gqa=%u gqaChunks=%u "
              "maxKv=%u maxActual=%u split=%u qkpv=%u kvSplit=%u kvTile=%u usedCore=%u "
              "dataCores=%u blockRange=%u tailRange=%u qTile=%d key=%lu logical=%lu ws=%zu",
              numTokens, taskCount, gqaGroup, gqaChunkCount, maxKvLen, maxActualSeqLen,
              splitMode, qkPvMode, kvSplitPart,
              kvTileRows, usedCoreNum, dataCores, blockSplitRange, tailSplitRange,
              static_cast<int>(qTileMode), tilingKey,
              PickTilingKey(splitMode, qkPvMode, qTileMode, gqaGroup),
              workspaces[0]);
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(TurboquantAttentionPaged4bit)
    .Tiling(TurboquantAttentionPaged4bitTilingFunc);
