...
class TurboquantPackKVForCacheV2ToCache {
public:
    __aicore__ inline explicit TurboquantPackKVForCacheV2ToCache(AscendC::TPipe* pipe, TqRotateMatmulOp* rotateMm)
    ...
    __aicore__ inline void Init(
        ...
        keyCacheGm_.SetGlobalBuffer(key_cache, (uint64_t)cacheSlots * numHeads * slot_w_k);
        valueCacheGm_.SetGlobalBuffer(value_cache, (uint64_t)cacheSlots * numHeads * slot_w_v);
...
private:
    ...
    __aicore__ inline void CopyOutPackedBatch(uint32_t start, uint32_t m) {
        ...
        for (uint32_t i = 0; i < m; ++i) {
            ...
            auto packedRow = packedBatch[i * packedStride_];
            if (linear < nVec_) {
                const uint64_t out_base = CacheOutBase(linear, slot_w_k_);
                if (out_base != TQ_INVALID_OFFSET) {
                    copy_packed_ub_to_gm(keyCacheGm_, out_base, packedRow, slot_w_k_);
                }
            } else {
                const uint32_t valueRow = linear - nVec_;
                const uint64_t out_base = CacheOutBase(valueRow, slot_w_v_);
                if (out_base != TQ_INVALID_OFFSET) {
                    copy_packed_ub_to_gm(valueCacheGm_, out_base, packedRow, slot_w_v_);
                }
            }
        }
...
extern "C" __global__ __aicore__ void turboquant_pack_kv_for_cache_v2_to_cache(
    GM_ADDR key,
    GM_ADDR value,
    GM_ADDR codebook,
    GM_ADDR rotation_t,
    ...
    TurboquantPackKVForCacheV2ToCache op(&pipe, &rotateMm);
    op.Init(
        key,
        value,
        codebookPtr,
