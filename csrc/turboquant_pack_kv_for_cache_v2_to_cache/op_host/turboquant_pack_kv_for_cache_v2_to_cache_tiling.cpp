...
#include "turboquant_pack_kv_for_cache_v2_to_cache_tiling.h"
...
static ge::graphStatus TurboquantPackKvForCacheV2ToCacheTilingFunc(gert::TilingContext* context)
{
    ...
    const int64_t* numHeadsPtr = attrs->GetAttrPointer<int64_t>(4);
    const int64_t* cacheSlotsPtr = attrs->GetAttrPointer<int64_t>(5);
    if (nVecPtr == nullptr || slotWKPtr == nullptr || slotWVPtr == nullptr || vecPerCorePtr == nullptr ||
        numHeadsPtr == nullptr || cacheSlotsPtr == nullptr) {
    ...
    if (nVec < 1 || vecPerCore < 1 || numHeads < 1 || cacheSlots < 1) {
    ...
    TurboquantPackKvForCacheV2ToCacheTilingData tilingData {};
    const ge::graphStatus tilingStatus = FillKfcCubeTiling(ascendcPlatform, tilingApi, mPad, tilingData.cubeTiling);
    ...
    // constant across nVec values, avoiding CANN runtime kernel re-compilation
    // and MIX scheduling conflicts between consecutive calls.
...
IMPL_OP_OPTILING(TurboquantPackKvForCacheV2ToCache)
    .Tiling(TurboquantPackKvForCacheV2ToCacheTilingFunc);
