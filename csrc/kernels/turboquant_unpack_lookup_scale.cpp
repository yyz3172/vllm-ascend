...
// Kernel1: packed uint8 -> y_hat fp16.
...
public:
    ...
    using CB_T = half;   // fp16 codebook
    using OUT_T = half;  // fp16 y_hat
                                ...
                                uint32_t nVec, uint32_t headSize, uint32_t packedBytes, uint32_t vecPerCore)
    {
        ...
        // idx bytes are headSize/2, but packedBytes can vary; just allocate packedBytes.
        pipe_->InitBuffer(packedBuf_, packedBytes_ * sizeof(PACKED_T));
...
private:
    __aicore__ inline void DecodeOne(uint32_t vecIdx, AscendC::LocalTensor<half> codebookLocal)
    {
        auto packedLocal = packedBuf_.Get<uint8_t>();
        AscendC::DataCopy(packedLocal, packedGm_[vecIdx * packedBytes_], packedBytes_);

        // Load norm bytes from local packed buffer and decode as fp16.
        // Python packs norm as torch.float16 then views as uint8, so bytes are little-endian.
        ...
        half norm_h = normHalfTensor.GetValue(0);
        ...
        for (uint32_t b = 0; b < headSize_ / 2; ++b) {
            ...
            // Use fp32 intermediates for better numerical stability.
            half y0 = codebookLocal.GetValue(hi);
            half y1 = codebookLocal.GetValue(lo);
            yLocal.SetValue(b * 2, (half)((float)y0 * (float)norm_h));
            yLocal.SetValue(b * 2 + 1, (half)((float)y1 * (float)norm_h));
        }
...
{
    AscendC::TPipe pipe;
    TurboquantUnpackLookupScale op(&pipe);
