...
#include "bit_residual_pack_kv_for_cache_tiling.h"
...
static ge::graphStatus BitResidualPackKvForCacheTilingFunc(gert::TilingContext* context)
{
    ...
    if (nVecPtr == nullptr || vecPerCorePtr == nullptr || numHeadsPtr == nullptr ||
        blockSizePtr == nullptr || numBlocksPtr == nullptr) {
    ...
    if (nVec < 1 || vecPerCore < 1 || numHeads < 1 || blockSize < 1 || numBlocks < 1) {
    ...
    BitResidualPackKvForCacheTilingData tilingData {};
...
IMPL_OP_OPTILING(BitResidualPackKvForCache)
    .Tiling(BitResidualPackKvForCacheTilingFunc);
