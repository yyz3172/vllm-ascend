...
private:
    ...
    // TurboQuant 旋转矩阵 Π = antiquantScale [headDim, headDim] fp16。MM1 前做 Q_rot = Π@Q (= Q@Π^T)。
...
{
    // TurboQuant: Q_rot = Q @ Π^T。CopyQ 后、CopyK 前改写 L1 Q。
    // Π 暂存 kpL1（随后 CopyK 盖掉）。Q_rot：L0C float --F322F16 Fixpipe CFG_NZ--> L1 Q。
    // Fixpipe mSize 用 mAct 勿用 pad。后续 MM1 FixpipeCToGM 显式设 NoQuant，无需 dummy 复位。
    ...
    for (uint32_t mOff = 0U; mOff < subMSize; mOff += M_BASE) {
        ...
        LoadBTransposeToL0(piAbId, piStageL1, headDim, 0U, headDim, 0U, headDim);
