#!/usr/bin/env bash
# Compile tools/bit_residual_fia_paged_k8v4/test_aclnn_bit_residual_fia_paged_k8v4.cpp
#
# Usage:
...
TOOLS_DIR="${ROOT_DIR}/tools/bit_residual_fia_paged_k8v4"
OUT="${TOOLS_DIR}/output/test_aclnn_bit_residual_fia_paged_k8v4"
SRC="${TOOLS_DIR}/test_aclnn_bit_residual_fia_paged_k8v4.cpp"
...
if [[ "${1:-}" == "--rebuild-op" ]]; then
    bash "${ROOT_DIR}/script/lcy/bit_residual_fia_paged_k8v4/rebuild_op.sh"
fi

if [[ ! -f "${CUSTOM_OP_INC}/aclnn_bit_residual_fia_paged_k8v4.h" ]]; then
    echo "[build_test] ERROR: missing ${CUSTOM_OP_INC}/aclnn_bit_residual_fia_paged_k8v4.h"
    echo "[build_test] Run: bash ${ROOT_DIR}/script/lcy/bit_residual_fia_paged_k8v4/rebuild_op.sh"
    exit 1
fi
