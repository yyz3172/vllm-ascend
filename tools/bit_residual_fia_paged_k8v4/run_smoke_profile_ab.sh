#!/usr/bin/env bash
# One-shot: full custom-op build + 2 smoke torch profiles (vector attn vs FIA) + summarize.
#
# Profile dumps are keyed by operator name for easy browsing:
#   ${XRX_K8V4_PROFILE_DIR}/smoke/BitResidualAttentionPagedK8v4/<stamp>/rank0_*_ascend_pt
#   ${XRX_K8V4_PROFILE_DIR}/smoke/BitResidualFiaPagedK8v4/<stamp>/rank0_*_ascend_pt
#   + .../<op>/latest -> latest stamp
#
# Usage (inside container vllm-ascend-lcy-v018):
#   bash tools/bit_residual_fia_paged_k8v4/run_smoke_profile_ab.sh
#   bash tools/bit_residual_fia_paged_k8v4/run_smoke_profile_ab.sh --skip-build
#   ENABLE_DECODE_FIA=0 bash tools/bit_residual_fia_paged_k8v4/run_smoke_profile_ab.sh
#
# Env overrides:
...
SMOKE_PY="${ROOT_DIR}/tests/e2e/singlecard/xrx_bit_residual_k8v4_smoke.py"
...
PROFILE_ROOT="${XRX_K8V4_PROFILE_DIR:-/root/l00856060/perflog2}/smoke"
...
run_one_profile() {
    ...
    set +e
    python "${SMOKE_PY}" 2>&1 | tee "${log_file}"
    local rc=${PIPESTATUS[0]}
    set -e
    if [[ ${rc} -ne 0 ]]; then
        echo "[ERROR] smoke profile ${label} failed rc=${rc}; see ${log_file}" >&2
        exit "${rc}"
    fi
}
