...
OUT=${1:-"${ROOT_DIR}/tools/turboquant_rotate_matmul_probe_perf"}
...
g++ -std=c++17 -O2 \
    ...
    -I"${ROOT_DIR}/csrc/turboquant_rotate_matmul_probe/op_host" \
    "${ROOT_DIR}/tools/turboquant_rotate_matmul_probe_perf.cpp" \
    -L"${TORCH_ROOT}/lib" \
    -L"${TORCH_LIBS_ROOT}" \
    ...
    -Wl,--no-as-needed -ltorch -ltorch_python -Wl,--as-needed \
    -ltorch_cpu -ltorch_global_deps -lc10 -l"${PY_LIB}" -lascendcl -ldl \
    -o "${OUT}"
