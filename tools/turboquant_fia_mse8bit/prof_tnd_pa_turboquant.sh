#!/usr/bin/env bash
# Build / run / msprof for TurboquantFiaMse8bit (vllm-ascend port of FIA TQ P0).
#
# Modes:
#   build       Rebuild custom op + compile C++ test
#   run         Run test binary (requires prior build)
#   msprof      msprof op profiling (+ optional Source analyze)
#   analyze     Run analyze_source_exec.py on latest OPPROF dump
#   all         build + run
#
# Usage:
#   bash tools/turboquant_fia_mse8bit/prof_tnd_pa_turboquant.sh build
#   bash tools/turboquant_fia_mse8bit/prof_tnd_pa_turboquant.sh run --kv=1000 --log=run.log
#   bash tools/turboquant_fia_mse8bit/prof_tnd_pa_turboquant.sh msprof --kv=1000 --skip-build
#   bash tools/turboquant_fia_mse8bit/prof_tnd_pa_turboquant.sh msprof --kv=1000 --source
#   bash tools/turboquant_fia_mse8bit/prof_tnd_pa_turboquant.sh analyze
set -euo pipefail

TOOLS="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "${TOOLS}/../.." && pwd)"
OUTPUT_BIN="${TOOLS}/output/test_aclnn_tq_fia_mse8bit"
PROF_ROOT="${TOOLS}/prof_output/tnd_pa_turboquant"
LOG_FILE="${LOG_FILE:-${TOOLS}/tq_fia_run.log}"
CUSTOM_OPP_PATH="${ROOT_DIR}/vllm_ascend/_cann_ops_custom/vendors/vllm-ascend"
KERNEL_NAME="TurboquantFiaMse8bit"
GOLDEN_DIR="${GOLDEN_DIR:-${TOOLS}/golden_ortho_random}"
ANALYZE_PY="${TOOLS}/analyze_source_exec.py"
...
WARM_UP="${WARM_UP:-0}"
# Default metrics (no Source). Use --source to add Source + ccec_g rebuild.
...
# Separate msprof output by kv length to avoid overwriting.
...
setup_input_env() {
    ...
    if [[ -d "${GOLDEN_DIR}" ]]; then
        [[ -f "${GOLDEN_DIR}/golden_query.bin" ]] && export Q_PATH="${Q_PATH:-${GOLDEN_DIR}/golden_query.bin}"
        [[ -f "${GOLDEN_DIR}/golden_key.bin" ]] && export KV_KEY_PATH="${KV_KEY_PATH:-${GOLDEN_DIR}/golden_key.bin}"
        [[ -f "${GOLDEN_DIR}/golden_value.bin" ]] && export KV_VALUE_PATH="${KV_VALUE_PATH:-${GOLDEN_DIR}/golden_value.bin}"
        [[ -f "${GOLDEN_DIR}/golden_pi.bin" ]] && export PI_PATH="${PI_PATH:-${GOLDEN_DIR}/golden_pi.bin}"
        [[ -f "${GOLDEN_DIR}/golden_gamma.bin" ]] && export GAMMA_PATH="${GAMMA_PATH:-${GOLDEN_DIR}/golden_gamma.bin}"
    fi
    echo "KV_SEQ_LEN=${KV_SEQ_LEN} ASCEND_DEVICE_ID=${ASCEND_DEVICE_ID}"
    echo "Q_PATH=${Q_PATH:-<default>} PI_PATH=${PI_PATH:-<default>}"
}
...
EOF
    # Forward optional golden paths if set
    for v in Q_PATH KV_KEY_PATH KV_VALUE_PATH PI_PATH GAMMA_PATH GOLDEN_OUT_PATH WS_DUMP_PATH; do
        if [[ -n "${!v:-}" ]]; then
            printf 'export %s=%q\n' "${v}" "${!v}" >> "${run_wrapper}"
        fi
    done
...
do_build() {
    ...
    if [[ "${SKIP_OP_BUILD}" != "true" ]]; then
        ...
        bash "${ROOT_DIR}/script/lcy/tq_fia_mse8bit/rebuild_op.sh" "${rebuild_args[@]}"
    else
...
do_run() {
    ...
    if [[ ! -x "${OUTPUT_BIN}" ]]; then
        ...
        echo "Hint: bash prof_tnd_pa_turboquant.sh build"
        exit 1
    fi
    ...
    grep -E 'workspace|ERROR|failed|success|Turboquant|mode|golden|KV_SEQ|tiling' "${LOG_FILE}" | head -40 || true
    return ${exit_code}
}
...
do_analyze() {
    ...
    if [[ ! -f "${dump_dir}/fdata" ]]; then
        echo ""
        echo "No Source fdata in ${dump_dir} (rebuild with --source / ccec_g to get per-line exec counts)."
        return 0
    fi
    echo ""
    echo "=== analyze_source_exec.py ==="
    python3 "${ANALYZE_PY}" "${dump_dir}" --label "kv${KV_SEQ_LEN}"
    local summary="${opprof_dir}/kv${KV_SEQ_LEN}.summary.txt"
    local per_file="${opprof_dir}/kv${KV_SEQ_LEN}.per_file.csv"
    if [[ -f "${summary}" ]]; then
        echo "--- ${summary} ---"
        cat "${summary}"
    fi
    if [[ -f "${per_file}" ]]; then
        echo ""
        echo "--- top-15 hot files (per_file) ---"
        python3 - <<'PY' "${per_file}"
import csv, sys
rows = list(csv.DictReader(open(sys.argv[1])))
print(f"{'pipe':4} {'pct':>7} {'exec':>14}  file")
for r in rows[:15]:
    print(f"{r['pipe']:4} {float(r['exec_count_pct_of_pipe']):7.2f} {int(r['exec_count_total']):14d}  {r['file']}")
PY
    fi
}
...
do_msprof_op() {
    ...
    # Source metric needs ccec_g; force op rebuild unless user skipped AND already has Source binary.
    if [[ "${WANT_SOURCE}" == "true" && "${SKIP_OP_BUILD}" == "true" ]]; then
        echo "WARN: --source with --skip-build: fdata only appears if op was built with ccec_g."
    fi
...
echo "=== TurboquantFiaMse8bit mode=${PROF_MODE} kv=${KV_SEQ_LEN} ==="
case "${PROF_MODE}" in
    build) do_build ;;
    run) do_run ;;
    msprof) do_msprof_op ;;
