...
RUNNER=${RUNNER:-"${ROOT_DIR}/ztmp/bit_residual_pack_k8v4_perf/bit_residual_pack_k8v4_perf"}
DEVICE=${DEVICE:-0}
SEQ_LEN=${SEQ_LEN:-512}
...
Common env overrides:
  ...
  Q_LENS, KV_LENS, BATCH_SIZE, PREFILL_TOKENS, DECODE_BATCH, PACK_TOKENS, SLOT_PATTERN
EOF
    exit 0
fi

if [[ ! -x "${RUNNER}" ]]; then
    echo "[run_bit_residual_512] missing runner: ${RUNNER}" >&2
    echo "[run_bit_residual_512] build it with: bash tools/build_bit_residual_pack_k8v4_perf.sh" >&2
    exit 1
fi
...
case "${scenario}" in
    ...
    mixed_both)
        ...
        ;;
    *)
        echo "[run_bit_residual_512] unknown scenario: ${scenario}" >&2
        "$0" --help >&2
        exit 1
        ;;
esac
