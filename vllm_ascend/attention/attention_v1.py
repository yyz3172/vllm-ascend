...
# Per decode-forward-step FIA NPU timing (requires PROFILE_FORWARD + PROFILE_FIA).
_DYNKV_FIA_PROFILE_ACC: dict[str, float] = {
    "calls": 0.0,
    "fia_ms_total": 0.0,
    "fia_ms_max": 0.0,
    "kv_tokens_sum": 0.0,
...
def dynkv_fia_profile_enabled() -> bool:
    """NPU Event on each FIA call; adds 32x sync/step — use only for diagnosis."""
    return (
        os.environ.get("VLLM_DYNKV_PROFILE_FORWARD", "0") == "1"
        and os.environ.get("VLLM_DYNKV_PROFILE_FIA", "0") == "1"
    )


# Per decode-forward-step PA NPU timing (PROFILE_FORWARD + PROFILE_PA, non-capture).
_DYNKV_PA_PROFILE_ACC: dict[str, float] = {
    "calls": 0.0,
    "pa_ms_total": 0.0,
    "pa_ms_max": 0.0,
    "kv_tokens_sum": 0.0,
...
def dynkv_pa_profile_enabled() -> bool:
    """PA profiling: graph replay NPU sync + eager ``_npu_paged_attention`` NPU Events."""
    return dynkv_profile_pa_enabled()


def dynkv_pa_profile_reset() -> None:
    for k in _DYNKV_PA_PROFILE_ACC:
        _DYNKV_PA_PROFILE_ACC[k] = 0.0


def dynkv_pa_profile_snapshot() -> dict[str, float]:
    calls = int(_DYNKV_PA_PROFILE_ACC["calls"])
    total = float(_DYNKV_PA_PROFILE_ACC["pa_ms_total"])
    kv_sum = float(_DYNKV_PA_PROFILE_ACC["kv_tokens_sum"])
    return {
        "pa_calls": float(calls),
        "pa_ms_total": total,
        "pa_ms_avg": total / calls if calls else 0.0,
        "pa_ms_max": float(_DYNKV_PA_PROFILE_ACC["pa_ms_max"]),
        "pa_kv_tokens_avg": kv_sum / calls if calls else 0.0,
    }


def _dynkv_pa_kv_tokens_max(context_lens: object) -> float:
    if context_lens is None:
        return 0.0
    if isinstance(context_lens, torch.Tensor):
        if context_lens.numel() == 0:
            return 0.0
        return float(context_lens.max().item())
    if isinstance(context_lens, (list, tuple)):
        if not context_lens:
            return 0.0
        return float(max(int(x) for x in context_lens))
    return float(context_lens)


def _dynkv_npu_paged_attention(**kwargs: object) -> None:
    """Call PA; optional NPU Event timing when PROFILE_PA and not capturing."""
    fc = get_forward_context()
    if getattr(fc, "capturing", False) or not dynkv_pa_profile_enabled():
        return torch_npu._npu_paged_attention(**kwargs)

    context_lens = kwargs.get("context_lens")
    ev0 = torch_npu.npu.Event(enable_timing=True)
    ev0.record()
    torch_npu._npu_paged_attention(**kwargs)
    ev1 = torch_npu.npu.Event(enable_timing=True)
    ev1.record()
    ev1.synchronize()
    ms = float(ev0.elapsed_time(ev1))
    _DYNKV_PA_PROFILE_ACC["calls"] += 1.0
    _DYNKV_PA_PROFILE_ACC["pa_ms_total"] += ms
    _DYNKV_PA_PROFILE_ACC["pa_ms_max"] = max(
        float(_DYNKV_PA_PROFILE_ACC["pa_ms_max"]), ms)
    _DYNKV_PA_PROFILE_ACC["kv_tokens_sum"] += _dynkv_pa_kv_tokens_max(
        context_lens)


def dynkv_fia_profile_reset() -> None:
    for k in _DYNKV_FIA_PROFILE_ACC:
        _DYNKV_FIA_PROFILE_ACC[k] = 0.0


def dynkv_fia_profile_snapshot() -> dict[str, float]:
    calls = int(_DYNKV_FIA_PROFILE_ACC["calls"])
    total = float(_DYNKV_FIA_PROFILE_ACC["fia_ms_total"])
    kv_sum = float(_DYNKV_FIA_PROFILE_ACC["kv_tokens_sum"])
    return {
        "fia_calls": float(calls),
        "fia_ms_total": total,
        "fia_ms_avg": total / calls if calls else 0.0,
        "fia_ms_max": float(_DYNKV_FIA_PROFILE_ACC["fia_ms_max"]),
        "fia_kv_tokens_avg": kv_sum / calls if calls else 0.0,
    }


def _dynkv_fia_kv_tokens_max(actual_seq_lengths_kv: object) -> float:
    if actual_seq_lengths_kv is None:
        return 0.0
    if isinstance(actual_seq_lengths_kv, torch.Tensor):
        if actual_seq_lengths_kv.numel() == 0:
            return 0.0
        return float(actual_seq_lengths_kv.max().item())
    if isinstance(actual_seq_lengths_kv, (list, tuple)):
        if not actual_seq_lengths_kv:
            return 0.0
        return float(max(int(x) for x in actual_seq_lengths_kv))
    return float(actual_seq_lengths_kv)


def _dynkv_npu_fused_infer_attention_score(
    *,
    actual_seq_lengths_kv: object,
    **kwargs: object,
) -> tuple[torch.Tensor, object]:
    """Call FIA; when forward profile is on, accumulate NPU Event ms per layer."""
    if not dynkv_fia_profile_enabled():
        return torch_npu.npu_fused_infer_attention_score(
            actual_seq_lengths_kv=actual_seq_lengths_kv, **kwargs)
    ...
    ev1.synchronize()
    ms = float(ev0.elapsed_time(ev1))
    _DYNKV_FIA_PROFILE_ACC["calls"] += 1.0
    _DYNKV_FIA_PROFILE_ACC["fia_ms_total"] += ms
    _DYNKV_FIA_PROFILE_ACC["fia_ms_max"] = max(
        float(_DYNKV_FIA_PROFILE_ACC["fia_ms_max"]), ms)
    _DYNKV_FIA_PROFILE_ACC["kv_tokens_sum"] += _dynkv_fia_kv_tokens_max(
...
class AscendAttentionBackendImpl(AttentionImpl):
    ...
    def _forward_fia_slidingwindow(self, query: torch.Tensor,
        ...
        output, _ = _dynkv_npu_fused_infer_attention_score(
            query=query,
            key=key,
            value=value,
            num_heads=self.num_heads,
    ...
    def forward_fused_infer_attention(self, query: torch.Tensor,
        ...
        # Get workspace from cache or calculate it if not present.
        attn_output, _ = _dynkv_npu_fused_infer_attention_score(
            query=query,
            key=key,
            value=value,
            atten_mask=attn_metadata.attn_mask,
