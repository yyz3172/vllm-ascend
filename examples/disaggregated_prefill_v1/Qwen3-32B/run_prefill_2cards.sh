#!/bin/sh
# 单机 2 卡 Prefill（TP=2），Qwen3-32B。基于 1card 版本，直接执行：bash run_prefill_2cards.sh
# ========== 配置区（仅改此处即可）==========
nic_name="eth0"
local_ip="172.17.0.2"
...
# 当前 Python 的 lib 目录（若报 libpython3.11.so.1.0 找不到，请填写 venv 的 lib 或 uv 的 Python lib，如 /root/autodl-tmp/py_venv/vllm2/lib）
python_lib_path="/root/.local/share/uv/python/cpython-3.11.15-linux-aarch64-gnu/lib"
# DP / 端口 / 设备（2 卡 P 一般无需改）
dp_size=1
dp_ip="127.0.0.1"
dp_port=13395
engine_port=9000
