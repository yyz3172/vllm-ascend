#!/bin/sh
# 单机 1 卡 Decode（TP=1），Qwen3-32B。直接执行：bash run_decode_1card.sh
# ========== 配置区（仅改此处即可，须与 run_prefill_1card.sh 中 model_path/nic_name/local_ip 一致）==========
nic_name="eth0"
local_ip="172.17.0.2"
...
# DP / 端口 / 设备（dp_port 须与 Prefill 的 dp_port 不同；Decode 用另一张卡）
dp_size=1
dp_ip="127.0.0.1"
dp_port=13495
engine_port=9010
